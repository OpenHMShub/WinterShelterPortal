select A.id, CONCAT(Format(A.timeStart,'h:mm tt'),' - ',Format(A.timeEnd,'h:mm tt')) as 'timeSlot', T.volunteerTaskName as 'role', 
CASE 
...
where A.volunteerJobsId = :jobId and CAST(A.timeStart AS DATE) = CAST(dateadd(day, 1, GETDATE()) AS DATE);